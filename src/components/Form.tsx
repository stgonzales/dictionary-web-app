...
const Input: Component<{ error: boolean, errorMessage: string } & JSX.InputHTMLAttributes<HTMLInputElement>> = (props) => {
  const [ fontState ] = useFontFamily()

  return (
    <div class="w-full hover:cursor-pointer flex flex-col gap-2">
      <div class={`flex items-center bg-neutral-200 dark:bg-neutral-700 focus-within:outline focus-within:outline-1 ${props.error ? 'focus-within:outline-secondary' : 'focus-within:outline-primary'} rounded-2xl peer`}>
        <input {...props} style={ {background: 'none'}} class={`w-full py-5 pl-6 focus:outline-none text-heading-s text-neutral-600 dark:text-neutral-100 font-bold placeholder:font-${fontState.fontFamily} font-${fontState.fontFamily} placeholder:text-neutral-600/25 dark:placeholder:text-neutral-200/25`}/>
      ...
      <Show when={props.error}>
        <small class={`font-${fontState.fontFamily} text-secondary text-heading-s invisible peer-focus-within:visible`}>{props.errorMessage}</small>
      </Show>
    </div>
  )
}